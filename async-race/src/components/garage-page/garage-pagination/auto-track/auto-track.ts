import './auto-track.scss';
import { BaseComponent } from '../../../base-component';
import {
  deleteCar, deleteWinner, getCar, getCars, getRace, toDrive, updateCar,
} from '../../../../shared/for-server';
import { allAuto, allWinnersAuto } from '../../../../shared/all-auto';
import { Pagination } from '../../../pagination/pagination';

export class AutoTrack extends BaseComponent {
  removeBtn!: HTMLButtonElement | null;

  auto: { name: string; color: string; id: number; };

  autoNameElement!: HTMLElement | null;

  autoColorElement!: HTMLElement | null;
  ...
  constructor(auto: { name: string, color: string, id: number }) {
    super('div', ['auto-track']);
    this.auto = auto;
    this.render();
    this.preStartRace();
  }
  ...
  preStartRace(): void {
    ...
    this.startBtn?.addEventListener('click', () => {
      this.startAuto();
      this.controlStartStopButtons();
    });
    this.stopBtn?.addEventListener('click', () => {
      this.resetPositionAuto();
      this.controlStartStopButtons();
    });
  }

  async startAuto(): Promise<void> {
    const raceParametres = await getRace(this.auto.id);
    this.getTime(raceParametres as unknown as { velocity: number, distance: number });

    this.car = this.element.querySelector('#car');
    if (this.car) (this.car as HTMLElement).style.transitionDuration = `${this.raceTime}ms`;
  ...
  async race(): Promise<void> {
    if (!this.car) return;
    this.car.classList.add('race');
    await toDrive(this.auto.id);
  }

  resetPositionAuto():void {
    if (!this.car) return;
    this.car.style.transitionDuration = '0ms';
    this.car.classList.remove('race');
  }

  getTime(raceParametres: { velocity: number, distance: number }) : void {
    this.raceTime = raceParametres.distance / raceParametres.velocity;
  }

  controlStartStopButtons(): void {
    if (this.startBtn?.getAttribute('disabled') === 'disabled') {
  ...
  async remove(): Promise<void> {
    await deleteCar(this.auto.id);

    allWinnersAuto.forEach(async (car) => {
      if (car.id === this.auto.id) await deleteWinner(this.auto.id);
    });
  ...
  async update(auto: { name: string, color: string, id: number }): Promise<void> {
    await updateCar(auto);
    this.auto = await getCar(auto.id);

    this.autoNameElement = this.element.querySelector('.auto-track__name');
    this.autoColorElement = this.element.querySelector('.auto-track__auto_color');

    if (this.autoNameElement) this.autoNameElement.textContent = `${this.auto.name}`;
    if (this.autoColorElement) this.autoColorElement.setAttribute('fill', this.auto.color);
  }

  render() : void {
    this.element.innerHTML = `
    <div class="auto-track__edit-delete-btn_container">
      <button class="auto-track__edit-btn" id="select-track-btn"> Select </button>
      <button class="auto-track__delete-btn" id="remove-track-btn"> Remove </button>
      <div class="auto-track__name"> ${this.auto.name} </div>
    </div>

    <div class="auto-track__control">
      <div class="auto-track__control_buttons">
        <button class="auto-track__control_buttons-start" id="startBtn"> A </button>
        <button class="auto-track__control_buttons-stop" id="stopBtn" disabled> B </button>
      </div>

      <div class="auto-track__auto" id="car">
        <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
          width="80px" height="60px" viewBox="0 0 1280.000000 640.000000"
          preserveAspectRatio="xMidYMid meet">
            <g class="auto-track__auto_color" transform="translate(0.000000,640.000000) scale(0.100000,-0.100000)"
              fill="${this.auto.color}" stroke="none">
      ...
      <div class="auto-track__finish"></div>
    </div>
    `;
  }

  static async updateGaragePageCars() : Promise<void> {
    await getCars();
    this.garageTitleElement = document.querySelector('h2');
    if (this.garageTitleElement) this.garageTitleElement.innerText = `Garage page (${allAuto.length})`;
  }
}
