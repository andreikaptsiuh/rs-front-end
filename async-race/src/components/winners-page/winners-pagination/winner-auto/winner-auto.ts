import './winner-auto.scss';
import { BaseComponent } from '../../../base-component';
import { allAuto, allWinnersAuto } from '../../../../shared/all-auto';
import { getWinners } from '../../../../shared/for-server';

export class WinnerAuto extends BaseComponent {
  winAuto: { name: string; color: string; id: number; } | undefined;

  static element: HTMLElement;

  static winnersTitleElement: HTMLHeadingElement | null;

  constructor(private auto: { id: number, wins: number, time: number }, private index: number) {
    super('div', ['winner-auto']);
    this.auto = auto;
    this.index = index;
    WinnerAuto.element = this.element;
    this.render();
  }

  static remove(): void {
    this.element.remove();
    WinnerAuto.updateWinnersPageCars();
  }

  static async updateWinnersPageCars() : Promise<void> {
    await getWinners();
    this.winnersTitleElement = document.querySelector('h2');
    if (this.winnersTitleElement) this.winnersTitleElement.innerText = `Winners page (${allWinnersAuto.length})`;
  }

  render(): void {
    allAuto.forEach((elem) => {
      if (elem.id === this.auto.id) this.winAuto = elem;
    });
    if (!this.winAuto) throw Error('hui');

    this.element.innerHTML = `
    <div class="winner-auto__container">
      <p> ${this.index} </p>
      <svg class="winner-auto__container_img" version="1.0" xmlns="http://www.w3.org/2000/svg"
            width="80px" height="60px" viewBox="0 0 1280.000000 640.000000"
            preserveAspectRatio="xMidYMid meet">
              <g transform="translate(0.000000,640.000000) scale(0.100000,-0.100000)"
                fill="${this.winAuto.color}" stroke="none">
